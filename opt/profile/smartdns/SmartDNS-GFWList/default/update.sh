...
cp -f /tmp/temp_gfwlist /srv/smartdns/overture_gfw_domain.list

# Update China List

curl -sSL https://raw.githubusercontent.com/17mon/china_ip_list/master/china_ip_list.txt > /tmp/china_ipv4.list
curl -A "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:6.0) Gecko/20100101 Firefox/6.0" -sSL https://asn.bgp.space/china6.html | grep "^[0-9a-z:\/]*<br>" | sed "s/<br>//g" > /tmp/china_ipv6.list
echo > /tmp/line
cat /tmp/china_ipv4.list /tmp/line /tmp/china_ipv6.list > /srv/smartdns/china_ip.list
cat /srv/smartdns/china_ip.list | sed "s/^/whitelist-ip &/g" > /srv/smartdns/whitelist-ip.conf